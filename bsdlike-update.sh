...
update_freebsd() {
	if ! command -v freebsd-update &>/dev/null; then
		if command -v sudo &>/dev/null; then sudo freebsd-update fetch && sudo freebsd-update install
		elif test ${UID} -eq 0; then 		freebsd-update fetch && freebsd-update install
		fi
	fi 
}
update_bsd() {
	if command -v sudo &>/dev/null; then 	sudo pkg update -f && sudo pkg upgrade
	elif test ${UID} -eq 0; then 			pkg update -f && pkg upgrade
	fi
}
clean_bsd() {
	if command -v sudo &>/dev/null; then 	sudo pkg autoremove && sudo pkg clean
