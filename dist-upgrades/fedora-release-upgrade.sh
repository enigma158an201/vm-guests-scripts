...
switchDownloadFedoraRelease() {
	...
	if [[ -n ${sRelease} ]]; then 			read -rp "Do you want to upgrade to ${sNextRelease} (y/n)? " -n 1 sYesNo								
		if [[ ${sYesNo} = "y" ]]; then 		#suExecCommand "dnf --setopt=deltarpm=false --assumeyes --refresh --releasever=${sNextRelease}"
			...
			if true; then 					suExecCommand "tmux send-keys -t supgradeF 'dnf system-upgrade download --releasever=${sNextRelease} --allowerasing --best' C-m" # --skip-broken #--setopt=keepcache=1
											suExecCommand "tmux send-keys -t supgradeF 'dnf system-upgrade --reboot || dnf system-upgrade reboot || dnf5 system-upgrade reboot' C-m"
